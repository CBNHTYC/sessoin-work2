...
operators ={
    ...
    'теле2' : [900, 901, 902, 908, 952, 953, 995],
    'мегафон' : [928, 929, 938, 999]
    }
...
def getInfoOfUsers(listOfUsers):
    ...
    for user in listOfUsers:
        ...
        if (count != 1000) and (totalCount - count > 0):
            ...
            tempListOfUsers = api.users.get(user_ids = localStrOfUsers, fields = 'contacts', v = 5.83)
            req()
...
def getGroupsOfUser(userID):
    ...
    localOffset = 0
    result = api.execute.getGroups(idUser = userID, offset = localOffset, v = 5.74)
    req()
    listOfGroups = []
...
def getGroupsOfUsersTwo(listOfUsers):
    ...
    for i in range(len(listOfUsers)):
        ...
        if (i+1) % 20:
            strOfIDs +=','
        if not (i+1) % 20:
            query = api.execute.getGroupsTwo(strOfUsers = strOfIDs, v = 5.74)
            req()
            strOfIDs = ''
            result = query['list']
            if t == 1:
